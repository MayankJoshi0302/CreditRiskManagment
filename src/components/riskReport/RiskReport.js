...
function RiskReportdb() {
  ...
  const values = ("state ", state.data);

  const scoreValue = ("statescore", state.data.Predictions[0].ConfidenceScore);
  ...
  const [modelList, setModels] = useState(
    state.data ? state.data.Predictions : []
  );
  const [activeModel, setActiveModel] = useState(
    state.data ? state.data.Predictions[0] : {}
  );
  ...
  function Handler() {
    ...
    setScore(state.data.Predictions[0].ConfidenceScore);
  }
